...
#
#------------------------------------------------------------------------
class MayaviFAMxlsxReport(Report):
    """
        ...
        for fh in self.__db.get_family_handles():
            ...
            for eventref in self.__db.get_family_from_handle(fh).event_ref_list:
                if eventref.ref in ci_evkeys:
                    ...
                    Place="NONE"
                    if family:
                        #date= get_date(self.__db.get_event_from_handle(eventref.ref))
                        year=self.__db.get_event_from_handle(eventref.ref).get_date_object().get_year()
                        Place = place_displayer.display_event(self.__db, self.__db.get_event_from_handle(eventref.ref), self.pl_format)
						#placetxt = place_displayer.display_event(self.__db, event, self.pl_format)
                        i+=1
                        if family.father_handle is not None:
                            VID= self.__db.get_person_from_handle(family.father_handle).gramps_id
                            gender=self.__db.get_person_from_handle(family.father_handle).gender
                            Vname= _nd.display_name(self.__db.get_person_from_handle(family.father_handle).get_primary_name())
                            VClan= _nd.primary_surname(self.__db.get_person_from_handle(family.father_handle).get_primary_name())
                            attrdic[fh].add((VID, VID, i, family.gramps_id, year, Vname, VClan, gender, Vname+' ['+VID+']', Place))
                        if family.mother_handle is not None:
                            MID= self.__db.get_person_from_handle(family.mother_handle).gramps_id
                            gender=self.__db.get_person_from_handle(family.mother_handle).gender
                            Mname= _nd.display_name(self.__db.get_person_from_handle(family.mother_handle).get_primary_name())
                            MClan= _nd.primary_surname(self.__db.get_person_from_handle(family.mother_handle).get_primary_name())
                            fedic[fh].add((VID, MID, i, family.gramps_id, year, Vname, Mname, VClan, MClan, "Partner"))
                            attrdic[fh].add((MID, MID, i, family.gramps_id, year, Mname, MClan, gender, Mname+' ['+MID+']', Place))
                        for ch in family.child_ref_list:
                            CID= self.__db.get_person_from_handle(ch.ref).gramps_id
                            gender=self.__db.get_person_from_handle(ch.ref).gender
                            Cname= _nd.display_name(self.__db.get_person_from_handle(ch.ref).get_primary_name())
                            CClan= _nd.primary_surname(self.__db.get_person_from_handle(ch.ref).get_primary_name())
                            fedic[fh].add((VID, CID, i, family.gramps_id, year, Vname, Cname, VClan, CClan, "Kind"))
                            attrdic[fh].add((CID, CID, i, family.gramps_id, year, Cname, CClan, gender, Cname+' ['+CID+']', Place))
                        break							
        
        for fh in fedic.keys():
            print(fedic[fh])
						
        self.doc.start_table("VISONETable", "SRC-VISONETable")
#        column_titles = [_("Prim_ID"), _("Gramps_ID"), _("LNr"), _("FamID"), _("Ev ID"),_("Event"), _("Date"), _("Year"), _("Primary") , _("Person"), _("Primary Clan"), _("Person Clan"), _("Event Place"), _("Roll")] 
        column_titles = [_("PrimID"), _("GrampsID"), _("LNr"), _("FamID"), _("Year"), _("Primary") , _("Person"), _("PrimaryClan"), _("PersonClan"), _("Rolle")]
        i = 0
        self.doc.start_row()
        ...
        self.doc.start_table("VISONETable", "SRC-VISONETable")
#        column_titles = [_("Prim_ID"), _("Gramps_ID"), _("LNr"), _("FamID"), _("Ev ID"),_("Event"), _("Date"), _("Year"), _("Primary") , _("Person"), _("Primary Clan"), _("Person Clan"), _("Event Place"), _("Roll")] 
        column_titles = [_("PrimID"), _("PrimID"), _("LNr"), _("FamID"), _("Year"), _("Primary") , _("Clan"), _("gender") , _("NameLong"), _("Ort")]
        i = 0
        self.doc.start_row()
        for title in column_titles:
            self.doc.start_cell("SRC-TableColumn")
        ...
        for fh in attrdic.keys():
            for setf in attrdic[fh]:
                self.doc.start_row()
                for item in setf:
                    # Prim ID
                    self.doc.start_cell("SRC-Cell")
...
#
#------------------------------------------------------------------------
class MayaviFAMxlsxReportOptions(MenuReportOptions):
    """
