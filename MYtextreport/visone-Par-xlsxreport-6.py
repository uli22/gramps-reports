        ...
        for event_ref in self.__db.get_person_from_handle(personhandle).get_event_ref_list():
            if int(event_ref.get_role()) == EventRoleType.PRIMARY:
                ...
                if int(event.get_type()) == EventType.RESIDENCE:
                    ...
                    print(mm)
        lptxt0 = (_("MIN%s///") % mm)
        lptxt = [", ".join(lp) for x in lp]
        lptxt = lptxt + [", ".join((_("%s") % lpdate)) for x in lpdate]
#        lptxt =lptxt0+lptxt
        print(lptxt)
...
#4.1        evkeys.sort
        ...
        column_titles = [_("Prim_ID"), _("Gramps_ID"),  _("Role"), _("Relation"), _("Kinship Path prim ->pers"), _("LNr"), _("Ev ID"),_("Event"), _("Date"), _("Year"), _("Age Primary"), _("Age Person"), _("gender Primary"), _("gender Person"),_("Primary") , _("Person"), _("Person birth"), _("Person Clan"),_("Primary Clan"),_("Event Place"),_("FamID"),_("FamID_P"),_("AnzKind"),_("Latitude"),_("Longitude")] 
        i = 0
        self.doc.start_row()
