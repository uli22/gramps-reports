...
#------------------------------------------------------------------------
#
# SourcesKonfirmationReport
#
#------------------------------------------------------------------------
class SourcesKonfirmationReport(Report):
    """
...
#        self.sort = Sort.Sort(self.database)
    ...
    def get_primary_event_ref_from_type(self, db, person, event_name):
        """
...
#        column_titles = [_("Prim_ID"), _("Gramps_ID"),  _("Role"), _("Relation"), _("LNr"), _("Ev ID"),_("Event"), _("Date"), _("Age Primary"), _("Age Person"), _("gender Primary"), _("gender Person") , _("Person"),_("Primary"), _("Person birth"), _("Person Clan"),_("Primary Clan"),_("Event Place")] 
        column_titles = [ _("LNr"), _("Person"), _("Gramps_ID"), _("gender Primary"),  _("Baptism"), _("Burrial"), _("Konfirmation"), _("Age Person_Burial"), _("Age Person_Konfirmation"),_("Event Place"), _("Baptism_Year"), _("Burrial_Year"), _("Konfirmation_Year"),_("Father"), _("Father_ID"),_("Mother"), _("Mother_ID"), _("Event ID"),_("Event Type"),_("Clan")] 
        i = 0
        self.doc.start_row()
        ...
        for ev in evkeys:
            ...
            for (a,b,c,d,e) in evdic[ev]:
                if c == EventRoleType.PRIMARY and self.__db.get_event_from_handle(b).get_type().is_baptism():
                    Fam = 0
                    name = d
                    ...
                    self.doc.start_paragraph("SRC-SourceDetails")
                    if name_handle:
                        burial_ref = self.get_primary_event_ref_from_type(self.__db, self.__db.get_person_from_handle(name_handle), "Burial")
                        if burial_ref:
                            burial = self.__db.get_event_from_handle(burial_ref.ref)
                            if burial:    
                                burialdate = get_date(burial)
...
#                                burialsource = get_primary_source_title(self.db, burial)
                    ...
                    self.doc.start_paragraph("SRC-SourceDetails")
                    if name_handle:
                        confirmation_ref = self.get_primary_event_ref_from_type(self.__db, self.__db.get_person_from_handle(name_handle), "Confirmation")
                        if confirmation_ref:
                            confirmation = self.__db.get_event_from_handle(confirmation_ref.ref)
                            if confirmation:    
                                confirmationdate = get_date(confirmation)
                                
                                place_handle = confirmation.get_place_handle()
                                if place_handle:
                                    place = self.__db.get_place_from_handle(place_handle)
                                    confirmationplace = place.get_title()
#                                confirmationsource = get_primary_source_title(self.db, baptism)
                    ...
                    # Age primary Person at burial            
                    self.doc.start_cell("SRC-Cell")
                    self.doc.start_paragraph("SRC-SourceDetails")
                    age = ""
                    if burialdate:
                        age = burial.get_date_object().get_year() - self.__db.get_event_from_handle(b).get_date_object().get_year()
                    self.doc.write_text(_(" %s") % age)
                            
                    self.doc.end_paragraph()
                    self.doc.end_cell() 
                                            
                    # Age primary Person at confirmation             
                    self.doc.start_cell("SRC-Cell")
                    self.doc.start_paragraph("SRC-SourceDetails")
                    age = ""
                    if confirmationdate:
                        age = confirmation.get_date_object().get_year() - self.__db.get_event_from_handle(b).get_date_object().get_year()
                    self.doc.write_text(_(" %s") % age)
                    ...
                    #Year of Event (burial)
                    self.doc.start_cell("SRC-Cell")
                    self.doc.start_paragraph("SRC-SourceDetails")
                    burialyear=""
                    if burial:
                        burialyear = burial.get_date_object().get_year()
                    self.doc.write_text(_("%s") %
                                 burialyear)
                    self.doc.end_paragraph()
                    self.doc.end_cell()
                    
                    #Year of Event (confirmation)
                    self.doc.start_cell("SRC-Cell")
                    self.doc.start_paragraph("SRC-SourceDetails")
                    confirmationyear = " "
                    if confirmation_ref:
                        confirmationyear = confirmation.get_date_object().get_year()    
                    self.doc.write_text(_("%s") %
                                 confirmationyear)
                    self.doc.end_paragraph()
                    self.doc.end_cell()
                    ...
                    father_name = " "
                    if father_handle:
                        father_name = self.__db.get_person_from_handle(father_handle).primary_name.get_name()

                    self.doc.write_text(_("%s") % father_name) 
                    self.doc.end_paragraph()
                    self.doc.end_cell() 
                    ...
                    father_ID = " "
                    if father_handle:
                        father_ID = self.__db.get_person_from_handle(father_handle).gramps_id

                    self.doc.write_text(_("%s") % father_ID) 
                    self.doc.end_paragraph()
                    self.doc.end_cell() 
                    ...
                    father_name = " "
                    if mother_handle:
                        mother_name = self.__db.get_person_from_handle(mother_handle).primary_name.get_name()

                    self.doc.write_text(_("%s") % mother_name) 
                    self.doc.end_paragraph()
                    self.doc.end_cell() 
                    ...
                    mother_ID = " "
                    if mother_handle:
                        mother_ID = self.__db.get_person_from_handle(mother_handle).gramps_id

                    self.doc.write_text(_("%s") % mother_ID) 
                    self.doc.end_paragraph()
                    self.doc.end_cell() 
...
#------------------------------------------------------------------------
#
# SourcesKonfirmationOptions
#
#------------------------------------------------------------------------
class SourcesKonfirmationOptions(MenuReportOptions):
    """
